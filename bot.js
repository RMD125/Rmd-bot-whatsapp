...
async function connectToWhatsApp() {
  ...
  sock.ev.on('messages.upsert', async (m) => {
    ...
    if (!message.key.fromMe && m.type === 'notify') {
      ...
      console.log(`📨 Message de ${pushname}: ${text}`);
      ...
      // Commandes Admin
      else if ((text.startsWith('!admin add ') || text.startsWith('.admin add ')) && isGroup) {
        await adminAdd(sock, jid, text, sender);
      }
      else if (text === '!admin list' || text === '.admin list') {
        await adminList(sock, jid);
      }
    }
  });
...
async function adminAdd(sock, jid, text, sender) {
  ...
  try {
    ...
    if (!isAdmin) {
      await sock.sendMessage(jid, { text: '❌ Vous devez être administrateur pour utiliser cette commande.' });
      return;
    }
    
    const number = text.split(' ')[2];
    if (!number) {
      await sock.sendMessage(jid, { text: '❌ Format incorrect. Utilisez: !admin add 22896190934' });
      return;
    }
    
    await sock.sendMessage(jid, { text: `✅ Le numéro ${number} a été ajouté à la liste des administrateurs.` });
  } catch (error) {
    console.error('Erreur adminAdd:', error);
  }
}
...
async function adminList(sock, jid) {
  try {
    ...
    let adminListText = '👑 *LISTE DES ADMINISTRATEURS*\n\n';
    admins.forEach((admin, index) => {
      adminListText += `${index + 1}. @${admin.id.split('@')[0]}\n`;
    });
    ...
    console.error('Erreur adminList:', error);
  }
}
